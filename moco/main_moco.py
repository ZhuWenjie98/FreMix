...
import datasetfolder as datasets
...
def train(train_loader, model, optimizer, scaler, summary_writer, epoch, args):
    ...
    source_losses = AverageMeter('Source Loss', ':.4e')
    mixsource_losses = AverageMeter('Mixsource Loss', ':.4e')
    mixmix_losses = AverageMeter('Mixmix Loss', ':.4e')
    progress = ProgressMeter(
        len(train_loader),
        [batch_time, data_time, learning_rates, source_losses,mixsource_losses, mixmix_losses],
    ...
    for i, (images, _) in enumerate(train_loader):
        ...
        if args.rank == 0:
            ...
            summary_writer.add_scalar("mixsource loss", mixloss_source.item(), epoch * iters_per_epoch + i)
            summary_writer.add_scalar("mixmix loss", mixloss_mix.item(), epoch * iters_per_epoch + i)
